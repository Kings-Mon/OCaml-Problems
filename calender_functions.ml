...
let days_in_month month year = match month with
...
let calgen year =
  ...
  let rec fill_calendar current_date =
    if current_date.year = year + 1 then
      ...
      Hashtbl.add calendar {dayofmonth=current_date.dayofmonth; monthnum=current_date.monthnum; year=current_date.year} weekday;
      let next_date =
